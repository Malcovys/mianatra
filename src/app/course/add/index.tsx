...
import { compileCourse, getOrCreateCourseImportSubject } from "@/src/presentation/features/course-import/services/course-import.service";
...
export default function AddCourseScreen() {
    ...
    listSubjects()
      .then((availableSubjects) => {
        ...
        const requestedSubject = requestedSubjectId
          ? availableSubjects.find((subject) => subject.id === requestedSubjectId)
          : null;
        if (requestedSubject) {
          setSubjectName(requestedSubject.name);
  ...
  async function chooseImages() {
    ...
    try {
      ...
      showError("Import impossible", message);
    } finally {
      setIsPicking(false);
    }
  }
  ...
  async function compileSelectedPages() {
    ...
    try {
      ...
      showError("Compilation impossible", message);
    } finally {
  ...
  return (
    ...
    >
      <ImportStepHeader label="Ajouter un cours"/>

      {/** affiche les matières déjà existantes sous forme de boutons */}
      {subjects.length > 0 ? (
      ...
      <CourseTextField
        ...
        value={subjectName}
        placeholder="Ex. Mathématiques ou SVT"
      ...
      <CourseTextField
        label="Classe"
        value={grade}
        placeholder="Ex. Seconde, Première ou Terminale"
      ...
      <AppButton
        ...
        loading={isCompiling}
        disabled={pages.length === 0 || !normalizedSubjectName || isPicking || isCompiling || Boolean(compiledCourseId)}
...
function CourseTextField({ label, value, placeholder, onChangeText }: CourseTextFieldProps) {
  return (
    <View className="gap-2">
      <AppText variant="label" className="text-[15px] leading-5">
        {label}
